...
const carouselImages = [background1, background2, background3, background4, background5, background6]
...
export default function ChalkHouse() {
  ...
  const bookTable = (e: React.FormEvent) => {
    ...
    if (selectedTable) {
      setTables(tables.map(table => 
        table.id === selectedTable.id ? { ...table, isAvailable: false } : table
      ))
      alert(`Table ${selectedTable.name} booked for ${bookingDate} at ${bookingTime}`)
  ...
  const buyWifiPackage = (packageId: number) => {
    ...
    if (selectedPackage) {
      alert(`You've purchased ${selectedPackage.duration} of WiFi for ${selectedPackage.price} KSH`)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <header className="bg-black text-white shadow-md fixed w-full z-10">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:block">
            <ul className="flex space-x-6">
              {['Home', 'About', 'Games', 'Challenges', 'Book', 'WiFi', 'Contact'].map((item) => (
                <li key={item}>
                  <a href={`#${item.toLowerCase()}`} className="text-white hover:text-red-500 transition-colors duration-300">
                    {item}
                  </a>
                </li>
              ))}
...
<section id="about" className="relative py-20 bg-gradient-to-br from-gray-50 to-white overflow-hidden">
  <div className="container mx-auto px-4">
    <h2 className="text-4xl font-bold mb-12 text-center text-gray-800 tracking-wide">About The Chalkhouse</h2>
...
>
  <div className="relative overflow-hidden rounded-lg shadow-2xl transform transition-transform hover:scale-105 duration-500">
    <img src={AboutImage} alt="The Chalkhouse Interior" className="object-cover w-full h-full" />
  </div>
</motion.div>
      ...
      >
        ...
        >
          The Chalkhouse is Nairobi's premier pool and billiards destination. We offer a sophisticated atmosphere for both casual players and serious enthusiasts.
        </motion.p>
        <motion.p
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
          ...
          className="text-lg leading-relaxed mb-6"
        >
          Our facility features top-of-the-line tables, a full-service bar, and a friendly staff dedicated to ensuring you have the best possible experience.
        </motion.p>
        <motion.p
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        >
          Whether you're looking to improve your game, compete in tournaments, or simply enjoy a night out with friends, The Chalkhouse is the perfect spot for you.
        </motion.p>
      </motion.div>
    </div>
  </div>
...
<section id="games" className="py-20 bg-gray-100">
  <div className="container mx-auto px-4">
    <h2 className="text-4xl font-bold mb-12 text-center text-gray-800">Our Games</h2>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {[
        ...
        >
          ...
          <div className="p-6">
            <p className="text-gray-700">
              Experience the thrill of {game.name} on our professional tables.
            </p>
          </div>
        </motion.div>
      ))}
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <form onSubmit={handleNewChallenge} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full transition-colors duration-300">
                Post Challenge
              </button>
            </form>
          </motion.div>
      ...
      <section id="book" className="py-20 bg-gradient-to-b from-gray-200 to-gray-300">
  <div className="container mx-auto px-4">
    <h2 className="text-4xl font-bold mb-12 text-center text-gray-800">Book a Table</h2>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h3 className="text-2xl font-semibold mb-4 text-gray-700">Available Tables</h3>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h3 className="text-2xl font-semibold mb-4 text-gray-700">Book Your Table</h3>
        <form onSubmit={bookTable} className="space-y-4">
          <div>
            <label htmlFor="table" className="block text-sm font-medium text-gray-700">Selected Table</label>
            <input
              type="text"
              id="table"
          ...
          <button
            ...
            className={`w-full py-2 px-4 font-bold text-white rounded-full transition-colors duration-300 ${
              selectedTable ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-400 cursor-not-allowed'
            }`}
      ...
      <section id="wifi" className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-4xl font-bold mb-12 text-center">WiFi Packages</h2>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start">
            ...
            >
              ...
              <form className="space-y-4">
                ...
                <button type="submit" className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full transition-colors duration-300">Send Message</button>
              </form>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, x: 50 }}
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <nav>
              <ul className="flex flex-wrap justify-center md:justify-end space-x-4">
                {['Home', 'About', 'Games', 'Challenges', 'Book', 'WiFi', 'Contact'].map((item) => (
                  <li key={item}>
                    <a href={`#${item.toLowerCase()}`} className="hover:text-red-500 transition-colors duration-300">{item}</a>
                  </li>
                ))}
              </ul>
            </nav>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex space-x-4 mt-4 md:mt-0">
              {['Facebook', 'Instagram', 'Twitter'].map((social) => (
                <a key={social} href="#" className="text-white hover:text-red-500 transition-colors duration-300">
